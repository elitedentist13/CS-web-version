...
function billListHasRecordableItems(items) {
    return (items || []).some(function (it) {
        ...
        return String(n.desc || '').trim() !== '' || (parseFloat(n.price) || 0) !== 0;
    });
}
...
function findBillRowForPendingList(pl, cb) {
    ...
    function tryLocalStore(next) {
        ...
        fetchBillRowForPendingList(bid, function (row) {
            ...
            // Keep unpaid and partial bills linked so item rows stay on the
            // editor (locked while any payment remains). Only drop fully settled.
            if (!billIsOpenForItemEditor(row)) {
                clearPendingBillIdLocalStore(pl);
                next();
                return;
            }
...
function buildUnpaidBillPayloadFromPendingList(pl, sub) {
    var payload = {
        ...
        total:          sub,
        amount_paid:    0,
        balance:        Math.max(0, sub),
        notes:          pendingListBillLinkNote(pl),
        status:         Math.max(0, sub) > 0.005 ? 'Partial' : 'Paid'
    };
...
function plusApptNotifyAppointmentSaved(meta) {
    ...
    if (cid) {
        ...
        if (typeof setWorkingClinic === 'function') {
            setWorkingClinic(cid, { syncFilters: true, reloadAppt: false });
        }
    }
    if (meta.doctorCode) {
        var drCode = String(meta.doctorCode).trim();
        if (drCode && typeof plusApptApplyDoctorCodeToApptBar === 'function') {
            plusApptApplyDoctorCodeToApptBar(drCode);
        }
...
function apptBannerOpenWhatsApp() {
    ...
    var msg = typeof trRepl === 'function'
        ...
        : ('Hello ' + name);
    openWhatsAppPrefill(phone, msg, { source: 'appointment' });
}

function apptBannerOpenBill() {
    var ap = (typeof activePatientSlots !== 'undefined' &&
              activePatientSlots[0] && activePatientSlots[0].id)
        ? activePatientSlots[0] : null;
    if (!ap || typeof openBillPanel !== 'function') return;
    openBillPanel({
        id: null,
        patient_id:   ap.id,
        patient_name: ap.full_name  || '',
        patient_no:   ap.patient_no || ''
    });
}

function refreshApptPatientOutstandingBalance(ap) {
    var balEl = g('apptBannerBalance');
    var btn   = g('apptBannerBalanceBtn');
    if (!balEl || !btn) return;
    if (!ap || !ap.id) { btn.style.display = 'none'; return; }
    ...
    var pno = String(ap.patient_no || '').trim();
    function fetchByPatientNo() {
        if (!pno) { hideBtn(); return; }
        SB.from('bills').select('balance, voided_at').eq('patient_no', pno)
            .then(function(r) { applyTotals(!r.error && r.data ? r.data : []); })
            .catch(function() { hideBtn(); });
    }
    SB.from('bills').select('balance, voided_at').eq('patient_id', ap.id)
        .then(function(r) {
            if (r.error) {
...
function updateApptPatientBanner() {
    var banner = g('apptPatientBanner');
    if (!banner) return;
    var ap = (typeof activePatientSlots !== 'undefined' &&
              activePatientSlots[0] && activePatientSlots[0].id)
        ? activePatientSlots[0] : null;
    ...
    if (!ap) {
        banner.classList.add('appt-patient-banner--no-patient');
        syncQueuePatientSearchInput();
        syncTodayPatientSearchInput();
        return;
    }
    banner.classList.remove('appt-patient-banner--no-patient');

    var nameEl = g('apptBannerName');
    if (nameEl) nameEl.textContent = ap.full_name || '—';

    var cnEl = g('apptBannerChinese');
    var cn = String(ap.chinese_name || '').trim();
    if (cnEl) {
        cnEl.textContent = cn;
    ...
    var noEl = g('apptBannerNo');
    if (noEl) noEl.textContent = ap.patient_no || '-';

    var sexWrap = g('apptBannerSexWrap');
    if (sexWrap) {
        var sexHtml = typeof patientSexSymbolHtml === 'function'
            ? patientSexSymbolHtml(ap.sex, { banner: true }) : '';
        var sexKind = typeof patientSexKind === 'function' ? patientSexKind(ap.sex) : 'unknown';
        if (sexKind === 'unknown') {
            sexWrap.style.display = 'none';
        } else {
            sexWrap.style.display = '';
            sexWrap.innerHTML = '<span data-i18n="con.banner.sexLabel"></span>&nbsp;' + sexHtml;
        }
    }

    var dobEl = g('apptBannerDob');
    if (dobEl) dobEl.textContent = ap.dob
        ? (typeof formatDobAge === 'function' ? formatDobAge(ap.dob) : ap.dob)
        : '-';

    var phoneEl = g('apptBannerPhone');
    if (phoneEl) phoneEl.textContent = ap.phone_number || ap.mobile_phone || '-';

    var mobWrap = g('apptBannerMobileWrap');
    var mobEl = g('apptBannerMobile');
    if (mobWrap && mobEl) {
        var mobStr = String(ap.mobile_phone || '').trim();
        var telStr = String(ap.phone_number || '').trim();
    ...
    var addrWrap = g('apptBannerAddressWrap');
    var addrEl = g('apptBannerAddress');
    if (addrWrap && addrEl) {
        var addrStr = String(ap.address || '').trim();
        if (addrStr) { addrWrap.style.display = ''; addrEl.textContent = addrStr; }
        else { addrWrap.style.display = 'none'; }
    }

    var emWrap = g('apptBannerEmailWrap');
    var emEl = g('apptBannerEmail');
    if (emWrap && emEl) {
        var emStr = String(ap.email || '').trim();
        if (emStr) { emWrap.style.display = ''; emEl.textContent = emStr; }
        else { emWrap.style.display = 'none'; }
    }

    var hkWrap = g('apptBannerHkidWrap');
    var hkEl = g('apptBannerHkid');
    if (hkWrap && hkEl) {
        var hkStr = String(ap.hkid || '').trim();
        if (hkStr) { hkWrap.style.display = ''; hkEl.textContent = hkStr; }
        else { hkWrap.style.display = 'none'; }
    }

    var todayEl = g('apptBannerToday');
    if (todayEl) todayEl.textContent = typeof fmtNowDateTimeHK === 'function'
        ? fmtNowDateTimeHK()
        : (typeof todayISO === 'function' ? todayISO() : '');

    var drEl = g('apptBannerDoctor');
    if (drEl) drEl.textContent =
        (typeof currentDoctorName !== 'undefined' && currentDoctorName) ||
        (typeof currentName !== 'undefined' && currentName) || '—';

    var alertWrap = g('apptBannerAlertWrap');
    var alertEl = g('apptBannerAlert');
    if (alertWrap && alertEl) {
        var alertTxt = '';
        if (typeof buildPatientAlertDisplayText === 'function') {
            alertTxt = String(buildPatientAlertDisplayText(ap) || '').trim();
        }
        if (!alertTxt) alertTxt = String(ap.medical_alerts || '').trim();
        if (alertTxt) { alertWrap.style.display = ''; alertEl.textContent = alertTxt; }
        else { alertWrap.style.display = 'none'; }
    }

    refreshApptPatientOutstandingBalance(ap);
    syncQueuePatientSearchInput();
    syncTodayPatientSearchInput();
}

// ─── Appointment Doctor Dropdown ──────────────────────────────────────────────
function populateApptDoctorSelect(clinicId) {
    var sel = g('apptDoctorSelect');
    if (!sel) return;
    var cId = clinicId || (typeof currentClinicId !== 'undefined' ? currentClinicId : '');
    var docs = typeof doctorsForClinic === 'function'
        ? doctorsForClinic(cId)
        : (typeof APP_DOCTORS !== 'undefined' ? APP_DOCTORS : []).filter(function(d) {
            return !cId || d.clinic_id === cId;
        });
    docs = (docs || []).filter(function(d) { return d.is_active !== false; });
    var prev = sel.value;
    sel.innerHTML = '<option value="">' + esc(tr('common.all')) + '</option>';
    docs.forEach(function(d) {
        var o = document.createElement('option');
        o.value = d.id;
        o.textContent = typeof doctorDisplayName === 'function'
            ? doctorDisplayName(d)
            : (d.english_name || d.doctor_code || '');
        sel.appendChild(o);
    });
    var cDr = typeof currentDoctorId !== 'undefined' ? (currentDoctorId || '') : '';
    var target = prev || cDr;
    var has = Array.prototype.some.call(sel.options, function(o) { return o.value === target; });
    sel.value = has ? target : '';
}

function onApptDoctorChange() {
    var sel = g('apptDoctorSelect');
    if (!sel) return;
    ...
    if (typeof plusApptSyncScopeFromApptBar === 'function') plusApptSyncScopeFromApptBar();
    var tab = typeof apptActiveTabKey === 'function' ? apptActiveTabKey() : '';
    if (tab === 'plusappt' && typeof onPlusApptDoctorChange === 'function') {
...
function bindApptDoctorSelectOnce() {
    var sel = g('apptDoctorSelect');
    if (!sel || sel.dataset.bound) return;
    sel.dataset.bound = '1';
    sel.addEventListener('change', onApptDoctorChange);
}

// ─── Queue Patient Search Bar ─────────────────────────────────────────────────
function _syncApptPatientSearchInput(inputId, clearBtnId) {
    var inp = g(inputId);
...
function syncQueuePatientSearchInput() {
    _syncApptPatientSearchInput('queuePsInput', 'queuePsClearBtn');
}

function syncTodayPatientSearchInput() {
    _syncApptPatientSearchInput('todayPsInput', 'todayPsClearBtn');
}
...
function bindQueuePatientSearchOnce() {
    _bindApptTabPatientSearchOnce('queuePsInput', 'queuePsDrop', 'queuePsClearBtn', 'queue-patient-search');
}

function bindTodayPatientSearchOnce() {
    _bindApptTabPatientSearchOnce('todayPsInput', 'todayPsDrop', 'todayPsClearBtn', 'today-patient-search');
}
...
function onApptClinicChange() {
    ...
    populateApptDoctorSelect(sel.value);
...
function initAppt(opts) {
    ...
    populateApptDoctorSelect(apSel ? apSel.value : '');
    bindApptDoctorSelectOnce();
...
function plusApptDoctorCodeFromApptBarSel() {
    var sel = g('apptDoctorSelect');
    if (!sel || !sel.value) return PLUSAPPT_DOCTOR_ALL;
    var doc = typeof getDoctorById === 'function' ? getDoctorById(sel.value) : null;
    if (doc && doc.doctor_code) return String(doc.doctor_code).trim();
    return PLUSAPPT_DOCTOR_ALL;
}

function plusApptSyncScopeFromApptBar() {
    var clinicSel = g('apptClinicSelect');
    plusApptActiveClinicId = (clinicSel && clinicSel.value)
        ? clinicSel.value
        : ((typeof currentClinicId !== 'undefined' && currentClinicId) ? currentClinicId : '');
    plusApptActiveDoctorCode = plusApptDoctorCodeFromApptBarSel();
    if (plusApptIsAllDoctorsMode()) {
        var docs = plusApptDoctorsForActiveClinic();
...
function plusApptApplyDoctorCodeToApptBar(code) {
    var sel = g('apptDoctorSelect');
    if (!sel) return;
    if (!code || code === PLUSAPPT_DOCTOR_ALL) {
        sel.value = '';
        return;
    }
    var docs = plusApptDoctorsForActiveClinic();
    for (var i = 0; i < docs.length; i++) {
        if (String(docs[i].doctor_code || '').trim() === String(code).trim()) {
            sel.value = docs[i].id;
            return;
        }
    }
...
/** Keep + Appointment scope aligned with the appointment module header bar. */
...
function populatePlusApptDoctorSelect() {
    var cid = plusApptActiveClinicId ||
        ((g('apptClinicSelect') && g('apptClinicSelect').value) || '') ||
        (typeof currentClinicId !== 'undefined' ? currentClinicId : '');
    if (typeof populateApptDoctorSelect === 'function') populateApptDoctorSelect(cid);
    plusApptSyncScopeFromApptBar();
}
...
function plusApptSyncTimelineHead() {
    ...
    var code = plusApptActiveDoctorCode;
    if (!code || code === PLUSAPPT_DOCTOR_ALL) {
    ...
    var docs = plusApptDoctorsForActiveClinic();
    for (var i = 0; i < docs.length; i++) {
        if (String(docs[i].doctor_code || '').trim() === code) {
            doc = docs[i];
            break;
        }
    }
    var name = doc
        ? ((typeof doctorDisplayName === 'function'
            ? doctorDisplayName(doc)
            : (doc.english_name || doc.chinese_name || code)) + ' [' + code + ']')
        : code;
    head.innerHTML =
...
function apptImportSoftRefreshPlusAppt(insertDateIso, importClinicTag, preferDoctorCode) {
    ...
    if (clinicId) {
        var clinicSel = g('apptClinicSelect');
        if (clinicSel && clinicSel.value !== clinicId) {
            clinicSel.value = clinicId;
        }
...
/**
 * Immediately freeze the elapsed timer badge on a queue row by updating the
 * row's data attributes to the new terminal status and refreshing all badges.
 * Called before the async DB update so the UI responds instantly.
 */
function queueFreezeRowImmediate(apptId, newStatus) {
    var tb = g('queueBody');
    if (!tb || !apptId) return;
    var row = tb.querySelector('tr[data-appt-id="' + String(apptId) + '"]');
    if (!row) return;
    row.dataset.billStatus = newStatus;
...
function buildQueueRow(tb, q, seqNo, dotCtx) {
    ...
    if (actDone) actDone.addEventListener('click', function(e) {
        queueStopRowEvent(e, true);
        queueCloseActionDrop(drop);
        queueFreezeRowImmediate(q.id, 'Done');
    ...
    if (actNoshow) actNoshow.addEventListener('click', function(e) {
        queueStopRowEvent(e, true);
        queueCloseActionDrop(drop);
        queueFreezeRowImmediate(q.id, 'No Show');
...
function refreshBillPanelLists(opts) {
    ...
    var runRefresh = function () {
        if (billStep2IsVisible()) {
            ...
            if (typeof syncPendingDraftFromInputs === 'function') syncPendingDraftFromInputs();
...
function loadPendingLists(cb) {
    ...
    .then(function(r) {
        ...
        merged.forEach(function (pl) {
            ...
            if (known && !billIsOpenForItemEditor(known)) {
                clearPendingBillIdLocalStore(pl);
                return;
            }
            pl.bill_id = localBid;
...
function renderStep1UI(opts) {
    opts = opts || {};
    ensureOpenBillsInPendingEditor(opts.focusBillId || null);
    ...
    // Also lock/unlock the Create Bill button, list label, doctor, and remove.
    ...
    if (labelEl) {
        labelEl.disabled = isLocked2;
        labelEl.readOnly = isLocked2;
    }
    var drSel = g('pendingListDoctor');
    if (drSel) drSel.disabled = isLocked2;
    var removeBtn = g('removePendingBtn');
    if (removeBtn) removeBtn.disabled = isLocked2 || !hasLists;
}
...
function addNewPendingList(opts) {
    opts = opts || {};
    ...
    if (!opts.skipFocus && g('pendingListLabel')) g('pendingListLabel').focus();
}
...
function createBillFromCurrentList() {
    ...
    if (!billListHasRecordableItems(billItems)) {
...
function saveCurrentPendingList(opts) {
    ...
    if (isCurrentPendingListLocked()) {
        if (opts.createBill) alert(tr('bill.alert.lockedBillNoEdit'));
        return;
    }
    var pl    = pendingLists[pendingIdx];
    ...
    function afterPendingListSaved(r) {
        ...
        function finishListSaved() {
            if (opts.createBill && billListHasRecordableItems(pl.items || billItems)) {
                if (statusEl) {
                    statusEl.textContent = tr('bill.status.creatingBill');
                    statusEl.style.color = '#888';
                }
                syncUnpaidBillFromPendingList(pl, sub, function (err, saved) {
                    releaseSaveLock();
                    if (err) {
                        if (statusEl) {
                    ...
                    var settled = saved && (parseFloat(saved.balance) <= 0.005 || (parseFloat(sub) || 0) <= 0.005);
                    function afterZeroPay() {
                        if (settled && saved && saved.id) {
                            resetBillCreationAfterPayment(saved.id);
                            if (typeof showBillDetail === 'function') showBillDetail(saved);
                            loadBillHistory(function () {
                                var latest = (billHistoryCache || []).find(function (b) {
                                    return b && String(b.id) === String(saved.id);
                                });
                                if (latest && typeof showBillDetail === 'function') showBillDetail(latest);
                            });
                            return;
                        }
                        renderStep1UI();
                    ...
                    if (settled && saved && saved.id) {
                        recordZeroBalanceSettlementPayment(saved, afterZeroPay);
                    } else {
                        afterZeroPay();
                    }
                });
...
/** After a bill is fully paid, wipe the item editor clean — do not unlock those rows.
 *  Keep the bill-detail payment panel open so the user lands there first. */
function resetBillCreationAfterPayment(billId) {
    ...
    var bid = String(billId || '');
    var linkedIds = [];
    pendingLists = (pendingLists || []).filter(function (pl) {
        if (!pl) return false;
        if (!bid || String(pl.bill_id || '') !== bid) return true;
        clearPendingBillIdLocalStore(pl);
    ...
    addNewPendingList({ skipFocus: true });

    linkedIds.forEach(function (id) {
        delete pendingServerSnapshotById[id];
        SB.from('pending_bill_items').delete().eq('id', id).then(function () {}).catch(function () {});
    });
}
...
function loadBillDoctors() {
    ...
    function applyDoctorOptions() {
        ...
        updateBillStep2DoctorSummary();
        var drSel = g('pendingListDoctor');
        if (drSel) drSel.disabled = isCurrentPendingListLocked();
    }
...
// When every payment is voided the bill unlocks and line items become editable.

function billIsOpenForItemEditor(b) {
    if (!b || !b.id || billRecordIsVoid(b)) return false;
    var bal = parseFloat(b.balance);
    if (!isFinite(bal)) {
        bal = (parseFloat(b.total) || 0) - (parseFloat(b.amount_paid) || 0);
    }
    return bal > 0.005;
}

function pendingListItemsFromBill(b) {
    return (parseBillItemsField(b && b.items) || []).map(function (it) {
        return normalizeBillItem(it);
    }).filter(function (it) {
        return it && (String(it.desc || '').trim() || (parseFloat(it.price) || 0) !== 0);
    });
}

function makePendingListFromOpenBill(b) {
    var items = pendingListItemsFromBill(b);
    var ref = b && b.id ? String(b.id).slice(0, 8).toUpperCase() : '';
    var date = (b && b.bill_date) ? String(b.bill_date) : '';
    var label = (date && ref) ? (date + ' · ' + ref) : (date || ref || trRepl('bill.list.defaultLabel', { N: '1' }));
    return {
        id: null,
        bill_id: b.id,
        label: label,
        items: items.length ? items : [{ desc: '', qty: 1, price: 0, disc: 0, tooth_no: '-' }],
        subtotal: parseFloat(b.subtotal) || pendingListSubtotalFromItems(items),
        doctor_id: (b && b.doctor_id) ? b.doctor_id : null,
        _hydratedFromBill: true
    };
}

/**
 * Keep unpaid / partial bills in the Step 1 editor.
 * Partial: item rows stay visible and locked.
 * After every payment is voided: same rows stay, now editable.
 */
function ensureOpenBillsInPendingEditor(preferBillId) {
    var cache = Array.isArray(billHistoryCache) ? billHistoryCache : [];
    if (!pendingLists) pendingLists = [];

    if (cache.length) {
        var openById = {};
        cache.forEach(function (b) {
            if (billIsOpenForItemEditor(b)) openById[String(b.id)] = b;
        });

        pendingLists = pendingLists.filter(function (pl) {
            if (!pl) return false;
            if (!pl.bill_id) return true;
            var bid = String(pl.bill_id);
            var open = openById[bid];
            if (open) return true;
            var cached = null;
            for (var i = 0; i < cache.length; i++) {
                if (cache[i] && String(cache[i].id) === bid) {
                    cached = cache[i];
                    break;
                }
            }
            if (cached) return false;
            return !pl._hydratedFromBill;
        });

        pendingLists.forEach(function (pl) {
            if (!pl || !pl.bill_id) return;
            var b = openById[String(pl.bill_id)];
            if (!b) return;
            var paid = parseFloat(b.amount_paid) || 0;
            var fromBill = pendingListItemsFromBill(b);
            if (paid > 0.005) {
                if (fromBill.length) {
                    pl.items = fromBill;
                    pl.subtotal = parseFloat(b.subtotal) || pendingListSubtotalFromItems(fromBill);
                }
                if (b.doctor_id) pl.doctor_id = b.doctor_id;
            } else if ((!pl.items || !pl.items.length) && fromBill.length &&
                    (pl._hydratedFromBill || !pl.id)) {
                pl.items = fromBill;
                pl.subtotal = parseFloat(b.subtotal) || pendingListSubtotalFromItems(fromBill);
            }
        });
        ...
        Object.keys(openById).forEach(function (id) {
            var exists = pendingLists.some(function (pl) {
                return pl && String(pl.bill_id) === String(id);
            });
            if (!exists) pendingLists.push(makePendingListFromOpenBill(openById[id]));
        });
        ...
        pendingLists.forEach(function (pl) {
            ...
            var prev = seen[k];
            if (!prev.id && pl.id) {
                var idx = deduped.indexOf(prev);
                if (idx >= 0) deduped[idx] = pl;
                seen[k] = pl;
            }
        });
        pendingLists = deduped;
    }

    if (preferBillId) {
        var hit = pendingLists.findIndex(function (pl) {
            return pl && String(pl.bill_id) === String(preferBillId);
        });
        if (hit >= 0) pendingIdx = hit;
    }
    if (!pendingLists.length) {
        pendingIdx = -1;
    } else if (pendingIdx < 0 || pendingIdx >= pendingLists.length) {
...
function syncPendingDraftFromInputs() {
    if (!pendingLists.length || pendingIdx < 0 || pendingIdx >= pendingLists.length) return;
    if (isCurrentPendingListLocked()) return;
    var pl = pendingLists[pendingIdx];
    if (!pl) return;
    var labelEl = g('pendingListLabel');
...
function saveBill(doPrint) {
    ...
    var finishAfterSaved = function(r) {
        ...
        // Keep the pending list (and its item rows) for unpaid / partial bills.
        // Only drop it once the bill is fully settled.
        if (paid > 0.005 && bal <= 0.005) {
            removePaidPendingList(paidPendingId, afterPendingRemoved);
        } else if (savedBillId && linkedPl) {
            persistPendingListBillIdRow(linkedPl, savedBillId, function () {
                afterPendingRemoved();
            });
        } else {
            afterPendingRemoved();
...
function billEligibleForPatientHistory(b) {
    ...
    if (Math.abs(total) > 0.005) return true;
...
function showBillDetail(b) {
    ...
    if (billIsOpenForItemEditor(b)) {
        renderStep1UI({ focusBillId: b.id });
    }
...
function recordZeroBalanceSettlementPayment(bill, cb) {
    if (!bill || !bill.id) {
        if (cb) cb();
        return;
    }
    var payRecord = {
        bill_id:     bill.id,
        paid_date:   bill.bill_date || (typeof todayISO === 'function' ? todayISO() : null),
        amount:      0,
        method:      bill.bill_type || (typeof billPendingPayTypeValue === 'function' ? billPendingPayTypeValue() : 'Pending'),
        notes:       (typeof billUserNotesText === 'function' ? billUserNotesText(bill.notes) : null) || null,
        received_by: (typeof currentName !== 'undefined' ? currentName : null)
    };
    if (typeof billPaymentClinicContext === 'function') {
        var clinicCtx = billPaymentClinicContext();
        payRecord.clinic_id = clinicCtx.clinic_id;
        payRecord.clinic_tag = clinicCtx.clinic_tag;
        payRecord.clinic_code = clinicCtx.clinic_code;
    }
    insertBillPaymentRecord(payRecord, function () {
        if (cb) cb();
    });
}
    ...
    insertBillPaymentRecord(payRecord, function(r) {
        ...
        .then(function(u) {
            ...
            if (newBalance <= 0.005) {
                ...
                if (bdCurrentBill && String(bdCurrentBill.id) === String(billId) &&
                        typeof showBillDetail === 'function') {
                    showBillDetail(bdCurrentBill);
                }
            } else {
                // Partial: keep item rows on the editor, locked, focused on this bill.
                renderStep1UI({ focusBillId: billId });
            }
            loadBillHistory(function() {
                if (newBalance <= 0.005) {
                    var latest = (billHistoryCache || []).find(function (b) {
                        return b && String(b.id) === String(billId);
                    });
                    if (latest && typeof showBillDetail === 'function') showBillDetail(latest);
                    return;
                }
...
function voidPaymentRecord(p) {
    ...
    .then(function(r) {
        ...
        .then(function(u) {
            ...
            if (typeof billHistoryCache !== 'undefined' && billHistoryCache && billHistoryCache.length) {
                billHistoryCache.forEach(function (b) {
                    if (!b || String(b.id) !== String(p.bill_id)) return;
                    b.amount_paid = newPaid;
                    b.balance = newBalance;
                    b.status = newStatus;
                    b.bill_type = nextBillType;
                });
            }
            if (bdCurrentBill && String(bdCurrentBill.id) === String(p.bill_id)) {
                bdCurrentBill.amount_paid = newPaid;
                bdCurrentBill.balance     = newBalance;
                bdCurrentBill.status      = newStatus;
            ...
            // Unlock + restore editable item rows when the last payment is voided;
            // keep rows locked while any installment remains.
            renderStep1UI({ focusBillId: p.bill_id });
            loadBillHistory(function() {
                renderStep1UI({ focusBillId: p.bill_id });
            });
            try { document.dispatchEvent(new CustomEvent('consultation-ar-refresh')); } catch (_) {}
